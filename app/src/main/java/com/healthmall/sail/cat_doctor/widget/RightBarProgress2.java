 ...
 */
public class RightBarProgress2 extends View {

    Paint progressPaint, textPaint, linePaint, nomarlPaint;

    float nomalHigh = 23.9f;
    float nomalLow = 18.5f;
    ...
    public RightBarProgress2(Context context) {
        super(context);
        initView();
    }

    public RightBarProgress2(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public RightBarProgress2(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        MLog.log("nomalLow:" + nomalLow + "  nomalHigh:" +
                nomalHigh + "  high1:" +
                high1 + "  high2:" +
                high2);
    ...
    private void drawBg(Canvas canvas) {
        ...
        float high2Y = getHeight() / 5f;
        canvas.drawLine(0, high2Y, 8, high2Y, linePaint);
        canvas.drawText(FloatUtils.getString(high2), 12, high2Y + textHeight / 2f, linePaint);

        float high1Y = getHeight() * 2f / 5f;
        canvas.drawLine(0, high1Y, 8, high1Y, linePaint);
        canvas.drawText(FloatUtils.getString(high1), 12, high1Y + textHeight / 2f, linePaint);

        float highY = getHeight() * 3f / 5f;
        canvas.drawLine(0, highY, 8, highY, linePaint);
        canvas.drawText(FloatUtils.getString(nomalHigh), 12, highY + textHeight / 2f, linePaint);


        float lowY = getHeight() * 4f / 5f;
    ...
    private void drawProgress(Canvas canvas, float progress, boolean isFinish) {
        ...
        Shader linearShader;
        if (progress > nomalHigh) { //超重

            if (progress > high2) { //重度肥胖
                progressY = getHeight() / 5f - getHeight() / 5f * (progress - high2) / (max - high2);

            } else if (progress > high1) { //肥胖
                progressY = getHeight() * 2 / 5f - getHeight() / 5f * (progress - high1) / (high2 - high1);
            } else { //偏胖
                progressY = getHeight() * 3 / 5f - getHeight() / 5f * (progress - nomalHigh) / (high1 - nomalHigh);
            }
            ...
            progressY = getHeight() - getHeight() / 5f * (progress - min) / (nomalLow - min);
            linearShader = new LinearGradient(25, progressY, 25, getHeight(), blueColor, transColor, Shader.TileMode.CLAMP);
        } else {
            progressY = getHeight() * 4f / 5f - getHeight() / 5f * (progress - nomalLow) / (nomalHigh - nomalLow);
